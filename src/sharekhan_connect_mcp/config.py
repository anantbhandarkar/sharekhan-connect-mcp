...
class Settings(BaseSettings):
    ...
    sharekhan_api_key: str = Field(..., env="SHAREKHAN_API_KEY", description="Your Sharekhan API Key")
    sharekhan_secret_key: str = Field(..., env="SHAREKHAN_SECRET_KEY", description="Your Secret Key for token decryption")
    sharekhan_customer_id: str = Field(..., env="SHAREKHAN_CUSTOMER_ID", description="Your Sharekhan Customer/Login ID")
    sharekhan_version_id: Union[str, int, None] = Field("1005", env="SHAREKHAN_VERSION_ID", description="API Version ID (null/1005/1006)")
    
    # Optional Sharekhan Configuration
    sharekhan_vendor_key: Optional[str] = Field(None, env="SHAREKHAN_VENDOR_KEY", description="Vendor Key (only for vendor login)")
    sharekhan_state: str = Field("12345", env="SHAREKHAN_STATE", description="OAuth state parameter")
    ...
    redirect_uri: str = Field("http://localhost:8080/auth/callback", env="REDIRECT_URI", description="OAuth redirect URI")

    # Logging Configuration
    log_level: str = Field("INFO", env="LOG_LEVEL", description="Logging level")
    log_file: str = Field("logs/sharekhan_mcp.log", env="LOG_FILE", description="Log file path")

    # API Configuration
    api_timeout: int = Field(30, env="API_TIMEOUT", description="API request timeout in seconds")
    max_retries: int = Field(3, env="MAX_RETRIES", description="Maximum retry attempts")
    retry_delay: float = Field(1.0, env="RETRY_DELAY", description="Delay between retries in seconds")

    # WebSocket Configuration
    ws_timeout: int = Field(60, env="WS_TIMEOUT", description="WebSocket connection timeout")
    ws_reconnect_attempts: int = Field(5, env="WS_RECONNECT_ATTEMPTS", description="Maximum reconnection attempts")
    ...
    def validate_configuration(self) -> dict:
        ...
        if not self.sharekhan_secret_key or self.sharekhan_secret_key == "your_secret_key_here":
